<?php
    ...
    include("nav.php");?>
<div class="container">
    <h2 class="title is-2" align="center">實驗室查詢</h2>
    <table class="table" align="center">
        <tr>            
            ...
            <td>
                <button class="button is-dark is-outlined" type="button" id="searchbtn" onclick="check_date()"  disabled="true">查詢</button>
            </td>
            <td>
                ...
                <select id="itemselect" name="item_category" onchange='droplist_search()'>
                    <option value="no">請選擇</option>
                        <?php
                            ...
                            $sql_s = "SELECT * FROM laboratory_category";
                            $sl = execute_sql($link,"borrow",$sql_s);
                        
                            while($row = mysqli_fetch_assoc($sl)){
                                echo "<option value=".$row['id']." >".$row['labName']."</option>";
                            }
    ...
    <div id="displaytable" style="position: relative;">
        <script>
            $("#displaytable").load("showpage/laball.php");
