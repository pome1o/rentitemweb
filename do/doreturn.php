...
<?php
    ...
    while($i<count($bno)) {
        ...
        // todo 寫入借用紀錄
        $sqlreturn = "INSERT INTO borowperson (personName,studentID,itemNo,lendTime) SELECT personName,studentID,itemNo,lendTime FROM tempborrow WHERE 
        itemNo = '$gg[0]' AND studentID = '$gg[1]'";
        
        // todo 將物品狀態改為可借用
	    $sql = "UPDATE item_info SET usingORnot = 0 WHERE  itemNo = '$gg[0]' ";
        //$sql_up = "UPDATE borowperson SET itemName = '$gg[2]' ";
        
        // todo 更新借用紀錄
        $sql_up = "UPDATE borowperson SET itemName = '$gg[2]' where itemNo = '$gg[0]' ";
        
        // todo 刪除暫存資料
        $sql_de = "DELETE FROM tempborrow WHERE itemNo = '$gg[0]' AND studentID = '$gg[1]' AND status = 1";
