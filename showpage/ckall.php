	...
	<form name="myForm" id="myForm" action="do/dock.php" method="post" >
         <table  align="center">
    <?php
    ...
    if(!isset($_GET['choice'])&&!isset($_GET['name'])){
        ...
        while ($row = mysqli_fetch_assoc($sl) and $j <= $per_page){
            ...
            echo "<input align='center' type='checkbox' id='itemBundle' name='itemBundle[]' value=".$row['itemNo'].'@@'.$row['studentID'].'@@'.$carray1[$d].">".$row['itemNo']."</td>";		   
    ...
    <?php 
        ...
        echo "<td >";
		echo "<input type='checkbox' data-act='checkall' data-scope='[id=itemBundle]' onclick='checkBoxAll()'>全選   ";
        echo "</td>";
        echo "<td >";
