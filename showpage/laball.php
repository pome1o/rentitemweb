...
if(!isset($_GET['choice'])){
    ...
    if($_GET['choice']=="no"){
        ...
        if($_GET['choice']=="name"){
            $id = $_GET['who'];
            $sql = "SELECT * FROM borrowlab WHERE studentID='$id'  ORDER BY lendTime ASC";
            borderset();
            showperson();
        }
        // todo 實驗室狀態
        else{
            $sql = "SELECT * FROM laboratory_info WHERE category='$c'";
            borderset();
            showlap();
        }
        ...
        if (mysqli_num_rows($result)>0){
            ...
            while($row = mysqli_fetch_assoc($result) and $j <=$per_page){
                ...
                echo "<td  align='center' bgcolor='#FFF2AF' >".$row['labNo']."</td>"; 
                //echo "<th style='padding:2px;' align='center' bgcolor='#FFF2AF' ><font size='5'>".$chshow."</font></th>"; 
                if($_GET['choice']=="name"){
                    echo "<td  align='center' bgcolor='#FFF2AF' >".$row['labName']."</td>";
                    echo "<td  align='center' bgcolor='#FFF2AF' >".$row['lendTime']."</td>";
                    echo "<td  align='center' bgcolor='#FFF2AF' ><font size='5'>".$row['returnTime']."</td>";
                }
                else{
                    //echo "<th style='padding:2px;' align='center' bgcolor='#FFF2AF' ><font size='5'>".$row['brand']."</font></th>"; 
                    if($row['usingORnot']!=0){
...
function borderset(){
    echo "<table  class='table' align='center'><tr   align='center'>";
}
