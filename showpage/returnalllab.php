...
<form name="myForm" id="myForm" action="do/doreturnlab.php" method="post" >
<table  align="center">
    <?php
    ...
    if($c=="allitem"){
        ...
        $sql_st = "SELECT b.studentID,b.personName,info.labNo,info.category FROM laboratory_info AS info,templab AS b WHERE b.itemNo = info.labNo AND b.status = 1 AND b.category = info.category";
    ...
    while ($row = mysqli_fetch_assoc($sl) and $j <=$per_page){
        ...
        echo "<input align='center' type='checkbox' id='bno' name='bno[]' value=".$row['labNo'].'@@'.$row['studentID'].'@@'.$carray1[$d].">".$row['labNo']."</td>";
